...
Cleos目前提供了使用eosio.msig合同提出操作的工具，但它没有提供一个简单的界面来提出自定义事务。因此，目前很难提出具有多个动作的原子事务 (for example `eosio::setcode` followed by `eosio::setabi`).

eosio.msig方法的优点是它使协调更容易，并且不对签名集合设置严格的时间限制（少于9小时）。

eosio.msig方法的缺点是要求提议者有足够的RAM来提出事务，而且当前cleos没有提供方便的工具来将它用于自定义事务，例如原子升级系统契约所必需的事务。
